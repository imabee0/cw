...
- **Picker non-interactivity is gated on `/dev/tty` + `stderr().is_terminal()`, never on `stdin().is_terminal()`** — crossterm reads keystrokes from `/dev/tty` directly and `tui::mod`'s `CrosstermBackend` renders to stderr, not stdout, so a piped stdin over a real controlling terminal must still reach the picker, and `cw | cat` must never receive UI escape codes. Verified empirically against the ratatui backend: `script -q /dev/null` allocates a real pty for local testing of this path.
