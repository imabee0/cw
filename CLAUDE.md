...
- **CI is one workflow** (`.github/workflows/ci.yml`). Cheap jobs (lint/deny, secrets, workflow-hygiene, dist-check) run in parallel. `build-test` `needs: lint` and runs only on `pull_request`/`workflow_dispatch`, so a merge to main does not rebuild macos. rust-cache is saved from main's lint job (`shared-key: 1.98`). Weekly cargo-deny is that same lint job on `schedule` — there is no separate Audit workflow.
- **Release chain is three files for mechanical reasons:** `release.yml` is cargo-dist-generated (`dist generate`); `release-plz.yml` needs `RELEASE_PLZ_TOKEN` so tag pushes re-trigger workflows (GitHub's anti-recursion rule: `GITHUB_TOKEN` never does); `release-verify.yml` listens on `workflow_run` of Release because a `GITHUB_TOKEN`-created GitHub Release never fires `on: release`. Never disable these. Never put a non-workflow YAML in `.github/workflows/` (`scripts/check.sh` and the `workflows` job reject it).
...
- **Release rustc is 1.98** (`rust-toolchain.toml`, `Cargo.toml` rust-version, `dist.rust-toolchain-version`) — GitHub macOS images ship an older rustup `stable`. Dist targets: `aarch64-apple-darwin`, `x86_64-unknown-linux-gnu`.
