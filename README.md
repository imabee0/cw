...
**Default flow** — pick a repo, clone/pull, create or resume a worktree, launch the selected agent. `cw` (bare), `cw resume`, and `cw clean` all open on a worktree pane listing every worktree across every repo. Click a row to focus it, double-click (or Enter) to open it; click an agent in the footer (or `←`/`→`) to choose claude/codex/grok/opencode/shell. Every pane supports arrow keys/`j`/`k`, live fuzzy filtering, and mouse click/scroll. In the worktree pane: `Space` marks a row, `d` opens a removal confirm (marked rows, or the focused row if none are marked). Confirm with `y`; worktrees with changed files or unpushed commits are kept unless you press `f` first (or started with `cw clean --force`). `r` rescans, `u` applies a pending self-update, `Esc` clears the status, then the filter, then the marked set, then cancels. Clone/pull and launch failures open an error modal (also written to `~/.cache/cw/cw.log.<date>`):
...
- `cw clean` / `cw clean --force` — remove finished worktrees (STATUS shows changed files / unpushed commits; `--force` starts the confirm including those). Same `Space`/`d` flow as any worktree pane.
- `cw doctor` — sanity-check the environment (`gh` auth, git credential helper, terminal, each configured agent on `PATH`); only a missing `default_agent` fails the exit code.
